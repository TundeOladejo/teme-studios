      ...
      <main>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 md:py-32 border-t border-[#E8DED2] text-[#2E231F]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 md:pt-16">
            {plans.map((pkg) => (
              <div key={pkg.name} className="border border-[#4A3933] p-4 md:p-10 flex flex-col gap-4 rounded-2xl text-[#4A3933]">
                ...
                <ul className="flex-1 space-y-1.5 my-8 divide-y divide-[#4A3933]">
                  {pkg.features.map((f, idx) => (
                    <li key={f} className={`text-[#4A3933] flex items-center gap-4 py-2 ${idx !== 0 ? "pt-1.5" : ""}`}>
                      <span className="w-3 h-3 bg-[#2E231F] inline-block flex-shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
                <Link
                  href={`/bookings?plan=${pkg.id}`}
                  className="tracking-widest border border-[#2E231F] text-[#2E231F] px-4 py-3 text-center hover:bg-[#2E231F] hover:text-white transition-colors uppercase"
                >
                  book now
                </Link>
              </div>
        ...
        <CTASection
          ...
          subtitle="Your journey to intentional movement, peaceful presence, and reconnection with yourself starts with a single session. We're here to hold space for you."
          secondaryHref=""
          secondaryLabel=""
          note="First-timers: We recommend starting with Solo Focus to experience our approach and find your preferred practice style."
        />
      </main>
      <Footer />
    </>
